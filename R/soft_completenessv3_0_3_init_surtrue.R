...
# 6) Inits
# ==================================================
# ----- Inits "proches vérité" -----
mu_true_mat <- do.call(rbind, mu)        # K x D
mu_init <- mu_true_mat + matrix(rnorm(K*D, 0, 0.15), K, D)  # petit jitter

Prec_init <- array(0, dim = c(D, D, K))
for (k in 1:K) {
  # précision = inverse(Sigma) + petit ridge pour éviter soucis SPD
  Prec_init[,,k] <- solve(Sigma[[k]]) + diag(1e-6, D)
}

pi_init <- (pi_true + 1e-3); pi_init <- pi_init / sum(pi_init)

inits1 <- list(mu = mu_init,
               Prec = Prec_init,
               pi = pi_init)

# deuxième chaîne : léger bruit supplémentaire
inits2 <- list(mu = mu_init + matrix(rnorm(K*D, 0, 0.10), K, D),
               Prec = Prec_init,
               pi = pmax(1e-3, pi_init + rnorm(K, 0, 0.02))); inits2$pi <- inits2$pi/sum(inits2$pi)
