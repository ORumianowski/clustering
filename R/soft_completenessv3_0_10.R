...
# sécuriser contre NA éventuels (bords)
...
code_naif <- nimbleCode({
  ...
  ## Priors sur mu (même squelette que corrigé mais sans ordre)
  for (k in 1:K) {
    for (d in 1:D) {
      mu[k,d] ~ dnorm(mu0[d], tau = Prec0[d,d])
    }
  ...
  ## Wishart séparés été/hiver + mapping bloc-diagonal (identique au corrigé)
  for (k in 1:K) {
    Prec_su[1:2,1:2,k] ~ dwish(R_su[1:2,1:2], df_su)
    Prec_wi[1:2,1:2,k] ~ dwish(R_wi[1:2,1:2], df_wi)
