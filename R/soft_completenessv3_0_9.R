...
#  Zeros-trick corrigé : phi = -ll + Cclip
# ==================================================
...
build_omega_winter <- function(xmin, xmax, ymin, ymax, nx = 60, ny = 60, mu, enlarge = 1.0, global_intensity = 0.3){
  center_wi_1 <- c(4, -26)
...
code_corrige <- nimbleCode({
  ...
  ## --- Prior ordonné sur mu[,1] (x été) ---
  mu1_base ~ dnorm(mu0[1], tau = prec_mu1_base)   # tau = précision
  mu[1,1]  <- mu1_base
  for (k in 1:(K-1)) {
    gap[k]  ~ dexp(lambda_gap)                    # écart > 0 (paramètre = rate)
    mu[k+1,1] <- mu[k,1] + gap[k]
  }
  
  ## --- Priors classiques sur les autres dimensions ---
  for (k in 1:K) {
    for (d in 2:D) {
      mu[k,d] ~ dnorm(mu0[d], tau = Prec0[d,d])   # tau = précision
    }
  }
...
constants <- list(
  ...
  Cclip = 300,
  Prec0=diag(D)*1e-2,
  R=diag(D), df=D+2,         # (non utilisés par les nouveaux priors)
  R_su=diag(2), R_wi=diag(2),
  df_su=2+2, df_wi=2+2,
  prec_mu1_base = 1/25,   # sd ≈ 5 pour la base (à ajuster si besoin)
  lambda_gap    = 1.0,     # E[gap] = 1 ; plus grand => gaps plus petits
  ss=as.integer(ss)
)
