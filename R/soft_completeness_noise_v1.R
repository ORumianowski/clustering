...
# --- A) Construire des Ω dégradés à partir des vrais rasters

# 1) Agrégation 
fact_agg <- 4
r_su_coarse <- terra::aggregate(r_su, fact = fact_agg, fun = mean, na.rm = TRUE)
r_wi_coarse <- terra::aggregate(r_wi, fact = fact_agg, fun = mean, na.rm = TRUE)

# 2) Fonction pour ajouter du bruit à 
add_noise <- function(r, amp){
  # Extraire les valeurs actuelles
  vals <- values(r)[, 1]  
  
  # Générer le bruit
  n_cells <- length(vals)
  eps <- runif(n_cells, min = -amp, max = amp)
  
  # Appliquer le bruit multiplicatif
  vals_noisy <- vals * (1 + eps)
  vals_noisy <- pmax(0, pmin(1, vals_noisy))
  
  # Remettre les valeurs dans le raster
  r_noisy <- r
...
# Appliquer le bruit
r_su_deg <- add_noise(r_su_coarse, amp = 0.35)
r_wi_deg <- add_noise(r_wi_coarse, amp = 0.35)
...
df_su_coarse <- raster_to_df(r_su_coarse, "Omega été agrégé")
df_wi_coarse <- raster_to_df(r_wi_coarse, "Omega hiver agrégé")
...
create_raster_plot <- function(df, title) {
  ggplot(df, aes(x = x, y = y, fill = value)) +
    geom_tile() +
    scale_fill_viridis_c(limits = c(0, 0.7), option = "plasma", 
                         name = "Omega") +
...
# Alternative : si vous voulez aussi voir les versions agrégées sans bruit
p5 <- create_raster_plot(df_su_coarse, "Omega été agrégé (15x15)")
p6 <- create_raster_plot(df_wi_coarse, "Omega hiver agrégé (15x15)")

# Plot comparatif : original vs agrégé vs dégradé pour été
plot_grid(p1, p5, p3, ncol = 3, labels = c("Original", "Agrégé", "Dégradé"))

# Plot comparatif : original vs agrégé vs dégradé pour hiver  
plot_grid(p2, p6, p4, ncol = 3, labels = c("Original", "Agrégé", "Dégradé"))
...
# --- B) Ω dégradés au niveau des observations (pour la vraisemblance) ---
