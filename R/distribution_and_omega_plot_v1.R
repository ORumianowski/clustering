...
# Avec relabellisation des clusters par x1 croissant (barycentre)
# ==============================================================
...
make_panel_title <- function(lbl) paste0("Ellipses — ", lbl, " ")

# --- helper relabel commun : applique une permutation à mu, Sigma, pi ---
relabel_by_perm <- function(mu_mat, Sigma_list, pi_vec, perm){
  list(
    mu    = mu_mat[perm, , drop = FALSE],
    Sigma = Sigma_list[perm],
    pi    = pi_vec[perm]
  )
}

# ---------- Fonction principale ----------
plot_results_mixture <- function(samples_corr4, samples_naif4, K,
                                 X4_all, sel_ete, sel_hiver,
                                 X_su_all, X_wi_all,
                                 mu_true = NULL, Sigma_true = NULL,
                                 pi_true = NULL, z_true = NULL,
                                 subplot_widths = c(1,1,1),
                                 panel_aspect = 0.9) {
  ...
  # --- Relabellisation par ordre croissant de x1 (colonne 1 de mu 4D) ---
  perm_c <- order(post_c$mu[, 1])
  perm_n <- order(post_n$mu[, 1])
  
  tmp_c <- relabel_by_perm(post_c$mu, post_c$Sigma, pi_c, perm_c)
  post_c$mu    <- tmp_c$mu
  post_c$Sigma <- tmp_c$Sigma
  pi_c         <- tmp_c$pi
  
  tmp_n <- relabel_by_perm(post_n$mu, post_n$Sigma, pi_n, perm_n)
  post_n$mu    <- tmp_n$mu
  post_n$Sigma <- tmp_n$Sigma
  pi_n         <- tmp_n$pi
  # ----------------------------------------------------------------------
  
  # Décomposition été / hiver après relabellisation
  ...
  # Segments pour individus observés
  ...
  if (has_truth) {
    # Construire mu_true_mat puis relabelliser par x1 croissant
    mu_true_mat <- do.call(rbind, mu_true)
    perm_t <- order(mu_true_mat[, 1])
    mu_true_mat <- mu_true_mat[perm_t, , drop = FALSE]
    Sigma_true  <- Sigma_true[perm_t]
    if (!is.null(pi_true)) {
      pi_true <- as.numeric(pi_true)[perm_t]
    }
    ...
    # Sous-titre π (vraies valeurs si dispo, sinon estimation via z, sinon message générique)
    pi_true_sub <- NULL
    if (!is.null(pi_true)) {
      pi_true_sub <- as.numeric(pi_true)
    } else if (!is.null(z_true)) {
      tabz <- prop.table(table(z_true))
      # réordonner les proportions selon perm_t si les labels de z_true sont 1..K
      tmp <- numeric(K)
      tmp[perm_t] <- as.numeric(tabz[as.character(1:K)])
      tmp[is.na(tmp)] <- 0
      pi_true_sub <- tmp
    }
    ...
    p_true <- build_pairs_plot(
      ...
      make_panel_title("VRAI"),
      if (!is.null(pi_true_sub)) fmt_pi(pi_true_sub) else "π (simulation)"
    )
    
    out <- (p_true | p_naif | p_corr) +
      plot_layout(guides = "collect", widths = subplot_widths) &
      theme(legend.position = "bottom")
  } else {
    out <- (p_naif | p_corr) +
      plot_layout(guides = "collect", widths = subplot_widths[1:2]) &
      theme(legend.position = "bottom")
  }
