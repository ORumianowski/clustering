...
# 6) SIX PLOTS :
#    (a) Points AVANT Ω   | (b) Points APRÈS Ω   | (c) Points sur fond Ω
#    (d) Ellipses RÉELLES | (e) Ellipses CORR(Ω) | (f) Ellipses NAÏVES
# ==================================================
...
K_c <- max(k_idx_c); D_c <- max(d_idx_c); stopifnot(K_c == K, D_c == D)
...
p_before <- ggplot(df_before, aes(x = x, y = y)) +
  geom_point(size = 0.6, alpha = 0.8, color = "steelblue4") +
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  theme_minimal(base_size = 12) +
  labs(title = "Points AVANT Ω", x = "x", y = "y")

# ---------- 6b) Points APRÈS Ω ----------
p_after <- ggplot(df_after, aes(x = x, y = y)) +
  geom_point(size = 0.6, alpha = 0.9, color = "black") +
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  theme_minimal(base_size = 12) +
  labs(title = "Points APRÈS Ω", x = "x", y = "y")

# ---------- 6c) Points sur fond Ω ----------
r_df <- as.data.frame(r, xy = TRUE); names(r_df)[3] <- "omega"
p_on_omega <- ggplot(r_df, aes(x = x, y = y, fill = omega)) +
  geom_raster() +
  geom_point(data = df_after, aes(x = x, y = y), inherit.aes = FALSE, size = 0.6) +
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  scale_fill_viridis_c(name = "Ω", limits = c(0,1)) +
  theme_minimal(base_size = 12) +
  labs(title = "Points sur fond Ω", x = "x", y = "y")
...
p_true <- ggplot() +
  ...
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  theme_minimal(base_size = 12) +
  labs(title = "Ellipses RÉELLES", x = "x", y = "y")

# ---------- 6e) Ellipses CORRIGÉES (avec Ω) ----------
p_corr <- ggplot() +
  ...
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  theme_minimal(base_size = 12) +
  labs(title = "Ellipses ESTIMÉES (corrigé Ω)", x = "x", y = "y")

# ---------- 6f) Ellipses NAÏVES (ignore Ω) ----------
p_naif <- ggplot() +
  ...
  coord_equal(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE) +
  theme_minimal(base_size = 12) +
  labs(title = "Ellipses ESTIMÉES (naïf)", x = "x", y = "y")

# ---------- Grille 2x3 ----------
(p_before | p_after | p_on_omega) /
  (p_true   | p_corr  | p_naif) +
