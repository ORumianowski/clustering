...
code_corrige <- nimbleCode({
  ...
  mixdens_min <- min(mixdens[1:N])
  mixdens_mean <- mean(mixdens[1:N])
  mixdens_max <- max(mixdens[1:N])
  
  Omega_min <- min(Omega[1:N])
  Omega_mean <- mean(Omega[1:N])
  Omegas_max <- max(Omega[1:N])
...
cat("➡️ Chaîne 1...\n")
samples1 <- runMCMC(cmcmc, niter = 1500, nburnin = 2, thin = 2,
                    inits = inits1, setSeed = 100)

cat("➡️ Chaîne 2...\n")
samples2 <- runMCMC(cmcmc, niter = 1500, nburnin = 2, thin = 2,
...
cat("➡️ Chaîne 1 (naïf)...\n")
samples_naif1 <- runMCMC(cmcmc_naif, niter = 1500, nburnin = 2, thin = 2,
                         inits = inits1, setSeed = 101)

cat("➡️ Chaîne 2 (naïf)...\n")
samples_naif2 <- runMCMC(cmcmc_naif, niter = 1500, nburnin = 2, thin = 2,
