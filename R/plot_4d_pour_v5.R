...
subplot_widths <- c(2, 2, 2)  # largeurs relatives (Vrai, Naïf, Corrigé). Ex: c(1.2, 1, 1)
panel_aspect   <- 0.9           # hauteur/largeur des panneaux (1 = carré, 0.8 = plus large que haut, etc.)
...
build_pairs_plot <- function(pairs_df, polys_df, title, subtitle, xlim_all, ylim_all,
  ...
  ggplot() +
    geom_segment(data = pairs_df,
                 ...
                 alpha=.06, linewidth=.5, show.legend = FALSE) +   # pas de légende pour les segments
    ...
    scale_color_manual(values=cluster_palette, guide = "none") +   # masque la légende "color"
    scale_alpha_manual(values=coverage_alphas, name="Contours prob.") +
    coord_fixed(ratio = aspect_ratio, xlim=xlim_all, ylim=ylim_all, expand=FALSE) +
...
p_before <- ggplot(df_before, aes(x, y, color = saison)) +
  ...
  theme_sq + labs(title = "Tous les individus (AVANT Ω)", x = "x", y = "y", color = "Saison")


# 2) Fonds Ω : même échelle + empilement (été en haut, hiver en bas)
# 2) Fonds Ω : même échelle + empilement (été en haut, hiver en bas) + titre global
su_r_df <- as.data.frame(r_su, xy = TRUE); names(su_r_df)[3] <- "omega"
wi_r_df <- as.data.frame(r_wi, xy = TRUE); names(wi_r_df)[3] <- "omega"

df_omega <- rbind(
  transform(su_r_df, saison = "ÉTÉ"),
  transform(wi_r_df, saison = "HIVER")
)
df_omega$saison <- factor(df_omega$saison, levels = c("ÉTÉ","HIVER"))

# mêmes limites & mêmes niveaux de contours pour les deux panneaux
lims_omega <- range(df_omega$omega, finite = TRUE)
max_omega  <- max(df_omega$omega, na.rm = TRUE)
brks_ct    <- c(0.5, 0.75, 0.9) * max_omega

p_omega_stack <- ggplot(df_omega, aes(x, y, fill = omega)) +
  geom_raster() +
  scale_fill_viridis_c(limits = lims_omega, name = expression(Omega)) +
  coord_equal(xlim = xlim_all, ylim = ylim_all, expand = FALSE) +
  facet_wrap(~ saison, ncol = 1, strip.position = "top") +
  theme_sq +
  theme(
    legend.position = "bottom",
  ...
  labs(
    title = "Effort d’échantillonnage – Ω ",  # Ω₂ en Unicode
    x = "x", y = "y"
  )
...
p_after <- ggplot(df_after, aes(x, y, color = saison)) +
  ...
  scale_color_manual(values = c("été" = "#F28E2B", "hiver" = "#4E79A7")) +
  coord_equal(xlim = xlim_all, ylim = ylim_all, expand = FALSE) +
  theme_sq + labs(title = "Individus observés (APRÈS Ω)", x = "x", y = "y", color = "Saison")

# 4) Mise en page
(fig_points <- (p_before | p_omega_stack | p_after))
