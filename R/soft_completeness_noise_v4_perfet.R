...
get_context_params <- function(ctx = c("C1","C2","C3")) {
  ...
  if (ctx == "C2") {  # contexte "milieu" (référence)
    mu_true <- list(
      c(-3.5,  0.0,  -1,   2),
      c( 0.0, -0.6,  -1,   2),
      c( 3.5,  0.0,   2,   5)
    )
    for (k in 1:3) mu_true[[k]][4] <- mu_true[[k]][4] - 30  # décalage hiver y
  ...
  if (ctx == "C3") {
    ## Été de C3 = Été de C2 (mêmes mu_su et même covariance su)
    ## Hiver de C3 = spécifique
    
    # Moyennes : (x_su, y_su) = C2 ; (x_wi, y_wi) spécifiques C3
    mu_true <- list(
      c(-3.5,  0.0,  0.2, -28.0),  # k1
      c( 0.0, -0.6,  0.3, -28.3),  # k2
      c( 3.5,  0.0,  0.0, -28.0)   # k3
    )
    
    # k1 : été = C2_k1 (sd_su = 0.7, 2.8 ; R_su diag) ; hiver spécifique (sd_wi = 0.7, 0.9 ; R_wi corr=0.10)
    sd1_C3 <- c(0.7, 2.8, 0.7, 0.9)
    R1_C3  <- matrix(c(
      1.00,  0.00,  0.00,  0.00,
      0.00,  1.00,  0.00,  0.00,
      0.00,  0.00,  1.00,  0.10,
    ...
    # k2 : été = C2_k2 (sd_su = 2.6, 2.4 ; R_su corr=-0.70) ; hiver spécifique (sd_wi = 0.7, 0.9 ; corr=0.10)
    sd2_C3 <- c(2.6, 2.4, 0.7, 0.9)
    R2_C3  <- matrix(c(
      1.00, -0.70,  0.00,  0.00,
      -0.70,  1.00,  0.00,  0.00,
      0.00,  0.00,  1.00,  0.10,
    ...
    # k3 : été = C2_k3 (sd_su = 0.8, 3.2 ; R_su diag) ; hiver spécifique (sd_wi = 1.1, 1.6 ; corr=0.25)
    sd3_C3 <- c(0.8, 3.2, 1.1, 1.6)
    R3_C3  <- matrix(c(
      1.00,  0.00,  0.00,  0.00,
      0.00,  1.00,  0.00,  0.00,
      0.00,  0.00,  1.00,  0.25,
    ...
    Sigma_true <- list(
      build_sigma(sd1_C3, R1_C3),
      build_sigma(sd2_C3, R2_C3),
      build_sigma(sd3_C3, R3_C3)
    )
  }
...
# ----------------------- Emprise commune (calculée une seule fois) -----------------------
...
pad <- 10   # <— ajuste ici la marge autour de l'emprise commune
...
plot_ctx_truth <- function(ctx = c("C1","C2","C3"),
                           panel_aspect = 0.9,
                           xlim = c(xmin_all, xmax_all),
                           ylim = c(ymin_all, ymax_all)){
  ctx <- match.arg(ctx)
  pars <- get_context_params(ctx)
  ...
  ggplot() +
    ...
    coord_fixed(ratio = panel_aspect, xlim=xlim, ylim=ylim, expand=FALSE) +
...
plot_raster <- function(df,
  ...
  ggplot(df, aes(x, y, fill = value)) +
    ...
    coord_fixed(xlim = xlim, ylim = ylim, expand = FALSE) +   # <— mêmes limites
...
# ----------------------- FIGURES -----------------------
# 1) Ellipses jolies pour les 3 contextes — avec les mêmes limites communes
pC1 <- plot_ctx_truth("C1")
pC2 <- plot_ctx_truth("C2")  # ton cas « milieu »
pC3 <- plot_ctx_truth("C3")
(pC1 | pC2 | pC3) + plot_layout(heights = c(1,1,1))

# 2) Ω : original + dégradé1 + dégradé2 (été & hiver) — même emprise commune
n_pix_side <- 60
