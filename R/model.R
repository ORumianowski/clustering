...
X_miss <- X
for(i in 1:n) {
  # Choisir combien de coordonnées garder pour cet individu
  n_obs <- sample(2:4, 1)   # 2, 3 ou 4 coordonnées observées
  missing_idx <- sample(1:d, d - n_obs)  # indices à rendre NA
  X_miss[i, missing_idx] <- NA
}

# Vérifier
apply(is.na(X_miss), 2, sum)   # combien de NA par colonne
...
# ============================================================
# 6. Évaluation : comparaison attribution modèle vs réalité
# ============================================================

# Fonction pour calculer l'accuracy avec permutation optimale des labels
...
acc <- classification_accuracy(z_true, z_est)
cat("\nTaux de points bien attribués :", round(acc * 100, 2), "%\n")
